...
AntiNK adalah sistem berbasis Docker dan FUSE yang bertugas mendeteksi file berbahaya di dalam direktori, dan memberikan perlakuan khusus seperti membalikkan nama file, mengenkripsi isi file, serta mencatat aktivitas ke log secara real-time. Sistem ini berjalan dalam container terisolasi menggunakan docker-compose.

a. Mendeteksi file berbahaya dan membalikkan nama
File yang mengandung kata kunci "nafis" atau "kimcun" dianggap berbahaya. Nama file tersebut akan dibalik saat ditampilkan.
...
b. Enkripsi isi file non-berbahaya dengan ROT13
file .txt biasa akan dienkripsi secara otomatis dengan algoritma ROT13 saat dibaca.
```bash
...
c. Adanya fitur pencatatan aktivitas
Semua aktivitas penting dicatat ke file log /var/log/it24.log dengan format timestamp.
```bash
...
Dengan menjalankan beberapa command :
- `docker-compose up --build` untuk build dan nge run docker
- `docker exec antink-server ls /antink_mount` melihat direktori hasil mount
